...
def save_splits(split_datasets, column_keys, filename, boolean_style=False):
	splits = [split_datasets[i].slide_data['slide_id'][:12] for i in range(len(split_datasets))]
...
class Generic_WSI_Classification_Dataset(Dataset):
	...
	def return_splits(self, from_id=True, csv_path=None):
		...
		if from_id:
			...
			print("HANIYAAAAAAAAAa ",all_splits,train_split,val_split,test_split,"******")
...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
	...
	def __getitem__(self, idx):
		...
		print("HELPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP ",slide_id)
		...
		if not self.use_h5:
			if self.data_dir:
				...
				if os.path.exists(full_path):
					...
					full_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id[:12]))
					print("ELSE PARTTTT ",full_path,label,slide_id)
					features = torch.load(full_path)
